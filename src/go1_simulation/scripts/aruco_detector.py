...
def publish_static_transform():
    ...
    q = tf_trans.quaternion_from_euler(-1.57079632679, 0, -1.57079632679)  # Roll, pitch, yaw
        ...
        for i, marker_id in enumerate(ids.flatten()):
            ...
            if ret:
                ...
                try:
                    ...
                    marker_msg.header = base_pose_stamped.header  # Use transformed pose header
                    marker_msg.id = marker_id
                    marker_msg.pose = base_pose_stamped.pose  # Transformed pose
                    ...
                    rospy.loginfo(f"Marker ID {marker_id}: Base Frame Pose: "
                                  f"X={base_pose_stamped.pose.position.x:.2f}, "
                                  f"Y={base_pose_stamped.pose.position.y:.2f}, "
                                  f"Z={base_pose_stamped.pose.position.z:.2f}")

                    # Publish the transformed pose
                    pose_pub.publish(base_pose_stamped.pose)

                except Exception as e:
                    rospy.logerr(f"Error transforming pose to base frame: {e}")
                    continue

                # Add marker ID to the list
                marker_ids.data.append(marker_id)
...
def main():
    ...
    rospy.Subscriber("/camera/color/faulty_image", Image, process_image)
